...
exports.isUnitRelative = isUnitRelative;
...
/**
 * Helper to get viewport size (depends in unit).
 *
 * @param  {String} unit Any unit
 * @return {Number} Viewport width or height
 */
function getViewportSize(unit) {
  var clientWidth = 0;
  var innerWidth = 0;
  var clientHeight = 0;
  var innerHeight = 0;
  ...
  if (unit === 'vw') {
    ...
    return Math.max(clientHeight, innerHeight);
  }
...
/**
 * Helper to evaluate if the unit used is relative to viewport
 *
 * @param  {String} unit Any unit
 * @return {Boolean}
 */
function isUnitRelative(unit) {
  switch (unit) {
    case 'vw':
    case 'vh':
      return true;
...
function setTransform(_ref) {
  ...
  var translate = 'translate(' + left + unit + ',' + top + unit + ')';
  return {
    transform: translate,
